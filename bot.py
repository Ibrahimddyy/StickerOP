...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔥 أهلاً بك في بوت الستيكرات\nاختار النوع من القائمة:", reply_markup=keyboard())
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📸 صورة":
        ...
        await update.message.reply_text("أرسل الفيديو الآن (أقل من 10 ثواني)")
        ...
        await update.message.reply_text(f"✅ تم حفظ الحزمة: {text}")
    elif text == "🗂️ حزمي":
        pack = user_packs.get(user_id)
        if pack: await update.message.reply_text(f"رابط حزمتك:\nhttps://t.me/addstickers/{pack}")
        else: await update.message.reply_text("لا توجد حزمة مفعلة.")
...
async def process(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id):
    ...
    try:
        if data.get("video"):
            ...
            # ضبط القياسات لتناسب تلغرام (أهم خطوة للستيكر المخفي)
            w, h = clip.size
            if w > h:
            ...
            out = f"{user_id}.webm"
            # audio=False ضروري جداً
            clip.write_videofile(out, codec="libvpx-vp9", fps=24, bitrate="400k", audio=False, logger=None)
            sticker_format = "video"
        else:
            img = Image.open(path).convert("RGBA")
            # إضافة خلفية بيضاء للصور لمنع الشفافية المزعجة
        ...
        if pack_name:
            with open(out, "rb") as f:
                ...
                try:
                    ...
                    # تم حل مشكلة sticker_format هنا
                    await context.bot.create_new_sticker_set(
                        user_id=int(user_id), name=pack_name, title="My Pack", stickers=[sticker_obj]
                    )
                    await update.message.reply_text("📦 تم إنشاء الحزمة!")
